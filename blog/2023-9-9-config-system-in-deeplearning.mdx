...
不同于前面两种配置系统使用 type，PaddleDetection 使用 yaml 的 [representer](https://github.com/PaddlePaddle/PaddleDetection/blob/release/2.7/ppdet/core/config/yaml_helpers.py#L50) 实现通过 `!{}` 来获取 python 对象。此外 PaddleDectection 在实现类时添加了类属性—— `__shared__` 和 `__inject__`，如：
...
PaddleSeg 中的配置格式并无特别之处，与前文相同，但是其解析、检查和构建对象都对进行了 [硬编码](https://github.com/PaddlePaddle/PaddleSeg/blob/release/2.9/paddleseg/cvlibs/builder.py)，这样的好处是编写代码时有补全。
...
为了一定程度上解决上述问题，我开发了 [ExCore](https://github.com/Asthestarsfalll/ExCore)，主要用于个人的实验。ExCore 使用 TOML 作为配置文件，并且借助 LSP（language server protocol）的力量在一定程度上弥补了配置文件和 python 文件之间的鸿沟——即为 TOML 配置文件提供了**补全、跳转、查看文档、参数检查**等功能，先看几个动图再介绍。

补全，查看文档和参数检查：

![config.gif](/images/2024/config.gif)

可以看到会有类名及其参数的补全以及查看对应类的 docstring，并且会对 `required` 参数进行提示。

跳转：

![to_class.gif](/images/2024/to_class.gif)

跳转以插件的形式实现，目前仅支持 NeoVim，见 [excore.nvim](https://github.com/Asthestarsfalll/excore.nvim)
...
这里的 `modules` 是一个 wrapper，可以通过**预先定义的主要字段**来获取构建的模块，如要获得 Model，只需要：

```python
model = modules.Model
```

但是目前还不支持补全，后续可以根据**主要字段**自动生成一个类专门用作 type-hinting，例如：
