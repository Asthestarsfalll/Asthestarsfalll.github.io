参考：https://www.mdpi.com/2379-139X/11/5/52
...
2023 年，元宇宙人工智能研究院（Meta AI Research）推出的分割一切模型（SAM）[129]，是一种强大的图像分割基础模型，无需任何标注数据即可适应新的任务和数据分布。分割一切模型经过大量高质量自动标注数据的训练，具备强大的零样本分割能力，为解决医学图像分割中手动标注图像稀缺的问题提供了新颖的解决方案，受到了该领域研究人员的广泛关注。
...
无监督医学图像分割方法旨在从数据本身提取内在结构和特征以实现分割，无需标注数据，从而省去了耗时费力的标注过程。然而，与监督学习方法相比，其分割准确性和稳定性仍存在差距。表 7 总结了 14 种代表性方法，重点介绍了它们的核心特征、优势、局限性和适用场景，为该领域的研究和应用提供了有价值的参考。
