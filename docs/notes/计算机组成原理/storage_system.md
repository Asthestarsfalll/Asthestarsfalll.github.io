...
1. [主存储器](./overview_of_computer_systems.md#五大功能部件)，又称内存。
2. 辅助存储器，又称辅存、外存。
...
4. 依靠程序的局部性原理。
**不同**
5. $Cache$ 主要解决系统速度，虚拟存储器解决主存容量；
6. $Cache$ 全由硬件实现，对所有程序员透明，而虚拟存储器由 $OS$ 和硬件共同实现，是逻辑上的存储器，对系统程序员不透明；
7. $CPU$ 速度约为 $Cache$ 的 10 倍，而主存速度为硬盘的 $100$ 倍，虚拟存储器不命中的影响大；
8. $Cache$ 可以直接和 $CPU$ 通信。
