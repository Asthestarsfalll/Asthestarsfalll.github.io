...
z=r\cos\phi,\ 0\leq \phi\leq 2\pi, \\
\end{cases}
$$
...
积分形式为 $\int f(x(t), y(t))\sqrt{ x'^2(t)+y'^2(t) } \, dx$ （适用于参数方程）或 $\int f(x,y(x)) \sqrt{ 1+y'^2(x) } \, dx$ 或 $f(r\cos\theta, r\sin\theta)\sqrt{ r^2 + r'^2 }d\theta$.
...
$$
\oint_{\sum_{out}}Pdyddz+Qdzdx+Rdxdy=\iiint\left( \frac{\partial P}{\partial x}+\frac{\partial Q}{\partial y}+\frac{\partial R}{\partial z} \right)dv=\iint_{\sum}(P\cos\alpha+Q\cos\beta+R\cos\gamma)dS
$$
