...
顺序存取，不能通过下标访问，在存取第N个数据时，必须先访问前（N-1）个数据 ，如链表.
...
为了保持一致，空表的头结点指向自身，则其判空条件就是头结点的前驱后继都指向自己.
...
|:-----|:-----|
|   size   |  返回元素的数量    |
|   empty   |   检查vector是否为空.如果为空，返回true；否则，返回false   |
|   clear   |   清空所有元素   |
...
|  reserve(new_capacity)    |   分配足够的内存以容纳至少new_capacity个元素，但不改变vector的大小   |
|  capacity    |  返回vector当前能够容纳的元素数量    |
